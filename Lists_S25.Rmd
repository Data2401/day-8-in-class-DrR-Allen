...
- a new logical operator -- `%in%` tells you if the object on the left can be found anywhere "in" the object on the right. 
...
- Lists are a common way to organize data, you can store employees' names, jobs, emails, and salaries all in one object 

- Like vectors, you can name the elements of the list. Unlike vectors, it's actually useful to do so in a list. 

## Creating a List

The `list()` function is how you make a list manually. You pass it any number of arguments, separated by commas, that you want in the list. (it's a lot like `c()` is for vectors... even though `vector()` is also a function.)
...
my_dogs <- list(
 ...
 weights = c(15, 45), 
 breed = c("dachshund", "definitely not a pit")
)
my_dogs
...
my_dogs_untagged <- list(
 ...
 c(TRUE, TRUE), 
 c(15, 45), 
 c("dachshund", "definitely not a pit"),
 list(toy = "ball", food = "chicken", thing = "naps")
)
# .... but they give necessary context
...
- Make a list that contains the information on a single playing card. Suit, face, and value. (Face and value aren't the same for all cards)
- Make a second list with three elements, the options for suit, face, and value, respectively. 
...
my_dogs[[2]] + 2   # This does not, adding 1 to the element stored inside
```

You can always store these elements as their own variable if you need to:
...
# I don't recommend doing this, tho, unless you're sure you'll never need the original again.
# Why? If you update the original list, these separate elements don't get updated, there is not a live link.
```
...
# in general, it's good practice to put these at the beginning of the document
